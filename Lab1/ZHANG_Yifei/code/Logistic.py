...
def main():
    display = 1
    # 创建 KNNClassifier 实例
    lr_classifier = Logistic('training_data.csv', 'songs_to_classify.csv', k=10)

    # 加载数据
    lr_classifier.load_data()
    ...
    # 预测
    predicted_labels = lr_classifier.predict(lr_classifier.test_data.values)

    print("Predicted Labels:", predicted_labels)

    if display == 1:
        lr_classifier.visualize_results()
