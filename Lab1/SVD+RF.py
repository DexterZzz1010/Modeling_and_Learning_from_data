...
class RFClassifier:
    ...
    def SVD(self, X, N=10):
        ...
        # 重构数据，仅保留前N个奇异值
        # reconstructed_data = np.dot(U_, S_, VT_)
        # reconstructed_data =  np.dot(U_k, S_k)
    ...
    def train(self):
        # 训练 模型
        self.model = RandomForestClassifier(n_estimators=100, oob_score=True, random_state=1234)
...
def main():
    display = 0
    # 创建 KNNClassifier 实例
    rf_classifier = RFClassifier('training_data.csv', 'songs_to_classify.csv')
